...
PH = {
 ...
 3: ([("3) Install CDP Private Cloud - Overview","58","on-premises.mdx","/on-premises"),
      ("3-1) Prerequisites Overview","58-59","on-premises/prerequisites.mdx","/on-premises/prerequisites"),
      ("3-2) Setup Repositories and Parcels","60-65","on-premises/repos-and-parcels.mdx","/on-premises/repos-and-parcels")],
     ["The source titles this section *3) Install CDP Private Cloud*; the `cdppvc` slug and image directory were renamed to **on-premises** on 2026-08-16, matching the current product name Cloudera on premises. Keep the source titles in this index and the new slugs in the repo.",
      "`on-premises.mdx` is a **section landing page** and must sit *beside* the `on-premises/` folder (sibling-file pattern), not inside it.",
 ...
 4: ([("3-3) Setup Cloudera Manager Server - Overview","65","on-premises/cm.mdx","/on-premises/cm"),
      ("3-3-1) Setup Database for Cloudera Manager","65-73","on-premises/cm/database.mdx","/on-premises/cm/database"),
      ("3-3-2) Install Cloudera Manager Server","73-76","on-premises/cm/server.mdx","/on-premises/cm/server")],
 ...
 5: ([("3-3-3) Enable Auto-TLS","77-80","on-premises/cm/auto-tls.mdx","/on-premises/cm/auto-tls"),
      ("3-3-4) Enable Kerberos","80-86","on-premises/cm/kerberos.mdx","/on-premises/cm/kerberos"),
      ("3-3-5) Setup Cloudera Management Services","87-89","on-premises/cm/mgmt-services.mdx","/on-premises/cm/mgmt-services"),
      ("3-3-6) Configure Cloudera Manager for external authentication using LDAP","90-95","on-premises/cm/ldap-auth.mdx","/on-premises/cm/ldap-auth")],
 ...
 6: ([("3-4) Setup CDP PvC Base Cluster - Overview","96","on-premises/base.mdx","/on-premises/base"),
      ("3-4-1) Install CDP PvC Base using the Cloudera Manager Web UI","96-105","on-premises/base/installation.mdx","/on-premises/base/installation")],
 ...
 7: ([("3-4-2) Data Lake Creation","105-119","on-premises/base/data-lake.mdx","/on-premises/base/data-lake"),
      ("3-4-3) Additional Requirements and Details","120-124","on-premises/base/additional-requirements.mdx","/on-premises/base/additional-requirements")],
 ...
 8: ([("3-4-4) Configure services with SSL/TLS-enabled Metadata Database","125-127","on-premises/base/services-metadata-db-tls.mdx","/on-premises/base/services-metadata-db-tls"),
      ("3-4-5) Scale the Cluster (Optional)","128","on-premises/base/scale-cluster.mdx","/on-premises/base/scale-cluster"),
      ("3-4-6) Enable High Availability (Optional)","128","on-premises/base/high-availability.mdx","/on-premises/base/high-availability"),
      ("3-4-7) Configure Services authentication for LDAP (Optional)","129-142","on-premises/base/ldap-auth.mdx","/on-premises/base/ldap-auth"),
      ("3-4-8) Optimize Log Collection","143","on-premises/base/optimize-log-collection.mdx","/on-premises/base/optimize-log-collection")],
     ["**Sections 3-4-5 and 3-4-6 share page 128** and must still become two separate pages. Read page 128 once and split it at the section heading — do not merge them.",
      ...
      "Note `on-premises/base/ldap-auth.mdx` (service auth, this phase) is a **different page** from `on-premises/cm/ldap-auth.mdx` (Cloudera Manager auth, Phase 5). Do not conflate them."]),
 ...
 12:([("5) Accessing Cloudera","191-198","accessing-data-services.mdx","/accessing-data-services")],
     ["The source titles this section *5) Accessing Cloudera*; the page was renamed **Accessing Data Services** on 2026-08-16, slug and image directory included, because every step on it is about the Data Services console. Keep the source title in this index and the new slug in the repo."]),
