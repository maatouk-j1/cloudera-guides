...
prevSlug: /installations/cdp-7-3-2/on-premises/base/data-lake
nextTitle: Configure services with SSL/TLS-enabled Metadata Database
nextSlug: /installations/cdp-7-3-2/on-premises/base/services-metadata-db-tls
---

The Base cluster is installed and healthy at the end of [Data Lake Creation](/installations/cdp-7-3-2/on-premises/base/data-lake). This page collects the adjustments its **Finish and verify** step promised: the service endpoints, the ports to open, the Web UI authentication changes, and the workstation host-file entries.
...
![Cloudera Manager Home page with the HDFS service outlined in red in the PvCBaseCluster1 service list, beside the cluster charts](/images/cdp-7-3-2/on-premises/base/additional-requirements/cm-home-hdfs.png)

![HDFS Configuration tab filtered on the search term kerberos, with the Enable Kerberos Authentication for HTTP Web-Consoles checkbox outlined in red and marked Uncheck this, and Save Changes highlighted at the bottom right](/images/cdp-7-3-2/on-premises/base/additional-requirements/hdfs-kerberos-webui-config.png)
...
<Banner type="warning">
**Note on Configure Kerberos on the previous page:** the DataNode ports that stage set were **1004** and **1006** - both below 1024, which is configuration (2) in the warning above. Item 3 here moves them to **9866** and **9864** and sets Data Transfer Protection to `Authentication`, which is configuration (1). Apply one or the other, not a mix.
</Banner>
...
52.221.202.246 hue-ptgty-hiv-wh1.apps.cldrsetup.local
52.221.202.246 cai-task-bo6klv.ptgty-cai-wb.apps.cldrsetup.local
52.221.202.246 ptgty-cai-wb.apps.cldrsetup.local
