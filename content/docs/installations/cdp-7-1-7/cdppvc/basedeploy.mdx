...
topicSlug: "cdppvc"
---
This article explains the necessary steps to install the minimum services on CDP PvC Base platform prior to installing the CDP PvC Data Services on the Kubernetes platform. Please ensure that all the [prerequisites](/installations/cdp-7-1-7/cdppvc/prerequisites) have been prepared and [CM](/installations/cdp-7-1-7/cdppvc/cm) has already been installed successfully before running this procedure.
...
1. Navigate to `Administration` > `Settings`. Search for `session timeout`. Key in `5 days`. This is a temporary setting to avoid session timeout during CDP PvC Base installation (You may revert this setting after successful installation). Log out and log in CM portal.
    ...
    ![Timeout](/images/cdp-7-1-7/cdpbase/timeout.png)
...
1. Navigate to `Clusters` > `Add Cluster`. 
   Select `Private Cloud Base Cluster` and click `Continue`.
    ...
    ![Addbase1 1](/images/cdp-7-1-7/cdpbase/addbase1-1.png)

    ![Addbase1 2](/images/cdp-7-1-7/cdpbase/addbase1-2.png)

2. Enter the Cluster Name and click `Continue`. 

    ![Addbase2](/images/cdp-7-1-7/cdpbase/addbase2.png)
...
9. Select `Custom Services`. Select `Atlas`, `HDFS`, `Hive`, `Ozone`, `Ranger`, `Yarn`, `Yarn Queue Manager` and `Zookeeper`. These are the minimum services needed on the CDP PvC Base cluster as the prerequisites to provision CDW, CML and CDE on Kubernetes platform later.
    ...
    ![Addbase9](/images/cdp-7-1-7/cdpbase/addbase9.png)
    
10. Select the node in the green boxes to host each service. Click `Continue`.

    ![Addbase10](/images/cdp-7-1-7/cdpbase/addbase10.png)
    
11. The outcome is similar to the following diagram. 

    ![Baseroles](/images/cdp-7-1-7/cdpbase/baseroles.png)
...
12. Fill in the database parameters based on the created databases in PostgreSQL. Click `Test Connection`. After getting successful result, click `Continue`.
    ...
    ![Addbase12](/images/cdp-7-1-7/cdpbase/addbase12.png)
    
13. Enter the required parameters. Click `Continue`.

    ![Addbase13](/images/cdp-7-1-7/cdpbase/addbase13.png)
    
14. Review and amend the parameters accordingly. Note that the directory path for services such as HDFS, Ozone and Zookeeper might require dedicated storage disk that is expected to be formatted and mounted prior to this installation as explained in the [prerequisites](/installations/cdp-7-1-7/cdppvc/prerequisites) subtopic. The directory must be configured here in accordance to the mounted disk folder name.

    ![Addbase14 1](/images/cdp-7-1-7/cdpbase/addbase14-1.png)

    For Ozone with SCM HA instances, configure hostname of one of the SCM hosts in the `Ozone SCM Primordial Node ID` field.
    
    ![Addbase14 2](/images/cdp-7-1-7/cdpbase/addbase14-2.png)
    
15. Click `Continue`.

    ![Addbase15](/images/cdp-7-1-7/cdpbase/addbase15.png)
    
16. The system will start to install the CDP Base services accordingly. Click `Continue` after successful installation.

    ![Addbase16](/images/cdp-7-1-7/cdpbase/addbase16.png)
...
17. All services should be in green mode as depicted below. Resolve the warning configuration issues or compress the warning if necessary.
    ...
    ![Basepost1](/images/cdp-7-1-7/cdpbase/basepost1.png)


---    
   Next Step
   
- Proceed to configure the necessary settings of CDP PvC Base cluster in the next [subtopic](/installations/cdp-7-1-7/cdppvc/baseconfig).