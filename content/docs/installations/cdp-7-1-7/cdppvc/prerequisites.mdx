...
- For CDP Private Cloud solution with Openshift platform, the required external components are represented by the purple-coloured components as shown in the following diagram.
    ...
    ![3partyocp](/images/cdp-7-1-7/3partyocp.png)
...
- For CDP PvC DS platform with ECS solution, the required external components are represented by the purple-coloured components as shown below.
    ...
    ![3partyecs](/images/cdp-7-1-7/3partyecs.png)
...
- The minimum CDP PvC Base services and its dependencies to install CML, CDW and CDE are illustrated in the table below.
    ...
    ![Base svc table1](/images/cdp-7-1-7/base_svc_table1.png)
...
- The external DNS server is expected to be ready prior to installing the CDP Private Cloud solution and its installation procedure is not covered in this article.
    ...
    ![Wildcarddns](/images/cdp-7-1-7/wildcarddns.png)
...
- The Openshift dashboard should reflect all components in green mode as depicted below.
    ![Ocp1](/images/cdp-7-1-7/ocp4/ocp1.png)    
...
- The `Local Storage Operator` must be provisioned to include the direct attached disk(s) for each Openshift worker node. See this [link](/installations/cdp-7-1-7/cdppvc/ocpcdwdisk) to explore the provisioning method.    
...
- The Openshift dashboard should reflect all OCS components in green mode as depicted below.
    ...
    ![Ocp2](/images/cdp-7-1-7/ocp4/ocp2.png)   
    
    ![Ocp3](/images/cdp-7-1-7/ocp4/ocp3.png)  
    
- Configure the OCS block storage PV as the default storageClass.

    ![Ocp4](/images/cdp-7-1-7/ocp4/ocp4.png)

### Docker Registry

- An external SSL-enabled Docker registry is required to store the Cloudera container images, e.g. Nexus Docker Registry. See this [link](/installations/cdp-7-1-7/cdppvc/nexus) to explore the sample methods to deploy the SSL-enabled Docker registry using Nexus solution.

### Hashicorp Vault

- An external SSL-enabled Hashicorp vault is required to store and encrypt the system's secrets and certificates. See this [link](/installations/cdp-7-1-7/cdppvc/vault) to explore the sample methods to deploy SSL-enabled Hashicorp Vault server.

## ECS Requirements

### Longhorn Storage

- Longhorn serves as the distributed block storage subsystem to persist data for the containers in the ECS platform. Each ECS node needs to be equipped with the direct-attached SSD/NVMe disk for the [Longhorn storage](https://longhorn.io/docs/1.2.4/best-practices/#minimum-recommended-hardware). Longhorn could only use a single volume disk per node and thereby [LVM](/installations/cdp-7-1-7/cdppvc/lvm) is recommended to be used for exposing a single volume backed by one/many physical disk.
...
- After preparing the abovementioned prerequisites, proceed to check out the steps to install Cloudera Manager in the subsequent [topic](/installations/cdp-7-1-7/cdppvc/cm).