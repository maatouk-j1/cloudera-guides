...
topicSlug: "cdppvc"
---
This article explains the necessary steps to install the ECS platform upon successful [deployment and configuration of CDP PvC Base cluster](/installations/cdp-7-1-7/cdppvc/base). Please ensure that all the [prerequisites](/installations/cdp-7-1-7/cdppvc/prerequisites) have already been prepared accordingly.
...
4. Ensure that ECS master node is able to resolve `console-cdp.apps.ecs1.cdpkvm.cldr` to the IP address of ECS master/server node or the Load Balancer (if being used).
    ...
    Address: 10.15.4.166
    ```
5. If Nvidia GPU card is involved, please install the Nvidia driver and its associated software as described in this [topic](/installations/cdp-7-1-7/cdppvc/gpuecs).

## ECS Platform Installation

1. Navigate to `Clusters` > `Add Cluster`. Select `Data Services` and click `Continue`.

    ![Addecs1](/images/cdp-7-1-7/ecs/addecs1.png)
    
2. Select `Internet` and click `Continue`. 

    ![Addecs2](/images/cdp-7-1-7/ecs/addecs2.png)
...
7. Select all ECS nodes in the `Docker Server` field. Select the ECS master node in the `Ecs Server` field and select the ECS worker nodes in the `Ecs Agent` field. Click `View By Host` to confirm your choices.   
    ...
    ![Addecs7](/images/cdp-7-1-7/ecs/addecs7.png)
    
    Click `Close` and `Continue`.
    
    ![Addecs8](/images/cdp-7-1-7/ecs/addecs8.png)
    
8. Click `Continue`.
    
    ![Addecs9](/images/cdp-7-1-7/ecs/addecs9.png)
    
9. Review and amend the parameters accordingly. Directory path for Longhorn storage and local SSD/NVMe (for CDW cache) in each node must be configured here in accordance to the mounted disk folder name. Dedicated storage disk that is expected to be formatted and mounted prior to this installation as explained in the [prerequisites](/installations/cdp-7-1-7/cdppvc/prerequisites) subtopic. The `Application Domain` field determines the subdomain of the wildcard DNS. In this case, the wildcard DNS to be set at the external DNS server is `*.apps.ecs1.cdpkvm.cldr`.
    
    ![Addecs10](/images/cdp-7-1-7/ecs/addecs10.png)
...
10. Fill in the database parameters based on the created databases in PostgreSQL. Click `Test Connection`. After getting successful result, click `Continue`.
    ...
    ![Addecs11](/images/cdp-7-1-7/ecs/addecs11.png)
    
    ![Addecs12](/images/cdp-7-1-7/ecs/addecs12.png)

12. CM is installing the parcels in each ECS node in parallel.

    ![Addecs13](/images/cdp-7-1-7/ecs/addecs13.png)
    
14. Select `Inspect hosts`. Check the results if needed. Otherwise, click `Continue`. 
    
    ![Addecs14](/images/cdp-7-1-7/ecs/addecs14.png) 
...
15. CM will proceed to install the ECS platform. This process will take some time to complete. After successful installation, click `Continue`. 
    ...
    ![Addecs15](/images/cdp-7-1-7/ecs/addecs15.png)  
    
16. Navigate to `Clusters` > `ECS 1`. Status of all components are supposed to be in green mode as depicted below.

    ![Addecs16](/images/cdp-7-1-7/ecs/addecs16.png)  
    
17. Click `ECS`. You may explore other ECS related dashboards such as `Storage UI` and `ECS Web UI`.  

    ![Addecs17](/images/cdp-7-1-7/ecs/addecs17.png)  
    
---    
   Next Step
   
- Proceed to configure CDP PvC Data Services in this [topic](/installations/cdp-7-1-7/cdppvc/ds).
