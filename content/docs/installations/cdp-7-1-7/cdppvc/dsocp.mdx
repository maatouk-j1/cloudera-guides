...
topicSlug: "cdppvc"
---
This article describes the steps to deploy the CDP Data Services Control Plane on the Openshift platform. Please ensure that all the [prerequisites](/installations/cdp-7-1-7/cdppvc/prerequisites) have already been prepared accordingly.
...
1. In Cloudera Manager portal, navigate to `Data Services` on the left panel and click `Add CDP Private Cloud Containerized Cluster`.
    ...
    ![Addocp1](/images/cdp-7-1-7/ocp4/addocp1.png)
    
2. Click `here` link.

    ![Addocp2](/images/cdp-7-1-7/ocp4/addocp2.png)   

3. Select the 1.3.4 version and click `Next`.

    ![Addocp3](/images/cdp-7-1-7/ocp4/addocp3.png)  
    
4. Select `Use a custom Docker Repository (Recommended for production)` option. Enter the fields based on the setup of the [Docker registry in the Nexus server](/installations/cdp-7-1-7/cdppvc/nexus).

    ![Addocp4](/images/cdp-7-1-7/ocp4/addocp4.png)     

5. Select `Use existing databases (Recommended for production)` option. Enter the fields based on the prerequisites highlighted in the [Installation Prerequisites](/installations/cdp-7-1-7/cdppvc/prerequisites) page.

    ![Addocp5](/images/cdp-7-1-7/ocp4/addocp5.png)  
    
6. Select the `kubeconfig` file as highlighted in the [Installation Prerequisites](/installations/cdp-7-1-7/cdppvc/prerequisites) page. 
For Vault configuration, enter the fields based on the setup of the [Hashicorp Vault](/installations/cdp-7-1-7/cdppvc/vault). 
For Storage, enter the default block storageClass of the deployed [OCS](/installations/cdp-7-1-7/cdppvc/prerequisites).

    ![Addocp6](/images/cdp-7-1-7/ocp4/addocp6.png)  
    
7. Click `Next`.

    ![Addocp7](/images/cdp-7-1-7/ocp4/addocp7.png)  
    
8. Click `Next`.   

    ![Addocp8](/images/cdp-7-1-7/ocp4/addocp8.png)  
    
9. Click `Finish`.    

    ![Addocp9](/images/cdp-7-1-7/ocp4/addocp9.png)  
    
10. The successful installation creates CDP PvC Control Plane on the Openshift platform as shown below.

    ![Addocp10](/images/cdp-7-1-7/ocp4/addocp10.png)  
